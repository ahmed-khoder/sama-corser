...
import { Plus, Trash2, Save, Package, Image, AlertCircle, X, Upload } from 'lucide-react';
    ...
    return (
        <div className="col-span-2 space-y-2">
            ...
            <div className="flex items-center gap-3">
                <label className="flex items-center gap-2 px-3 py-1.5 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 transition cursor-pointer text-xs font-medium">
                ...
                <span className="text-[10px] text-gray-400">{isRTL ? 'أو أدخل رابط مباشر بالأسفل' : 'or enter URL directly below'}</span>
            </div>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            ...
                            <div className="col-span-2 grid grid-cols-3 gap-3 pt-2 border-t border-gray-100 dark:border-gray-700">
                                <div>
                                    ...
                                    <input type="range" min="0" max="1" step="0.05" value={newCargo.heroOverlayOpacity ?? 0.2} onChange={e => setNewCargo(p => ({ ...p, heroOverlayOpacity: parseFloat(e.target.value) }))} className="w-full accent-emerald-600" />
                                    <span className="text-xs text-gray-400 block text-center">{newCargo.heroOverlayOpacity ?? 0.2}</span>
                                </div>
                                <div>
                                    <label className="text-xs text-gray-500 block mb-1">{isRTL ? 'ضبابية الجو' : 'Fog (px)'}</label>
                                    <input type="range" min="0" max="20" step="1" value={newCargo.heroBlurAmount ?? 0} onChange={e => setNewCargo(p => ({ ...p, heroBlurAmount: parseInt(e.target.value) }))} className="w-full accent-emerald-600" />
                                    <span className="text-xs text-gray-400 block text-center">{newCargo.heroBlurAmount ?? 0}px</span>
                                </div>
                                <div>
                                    <label className="text-xs text-gray-500 block mb-1">{isRTL ? 'لون الطبقة' : 'Overlay Color'}</label>
                                    <div className="flex items-center gap-2">
                                        <input type="color" value={newCargo.heroOverlayColor || '#0369a1'} onChange={e => setNewCargo(p => ({ ...p, heroOverlayColor: e.target.value }))} className="w-8 h-8 rounded border cursor-pointer" />
                                        <input value={newCargo.heroOverlayColor || '#0369a1'} onChange={e => setNewCargo(p => ({ ...p, heroOverlayColor: e.target.value }))} className="flex-1 px-2 py-1 rounded-lg border dark:bg-gray-800 dark:border-gray-700 text-xs" placeholder="#0369a1" />
                                    </div>
                                </div>
                            </div>
                        </div>
                            ...
                            <div className="space-y-3">
                                <div className="grid grid-cols-2 gap-3">
                                    ...
                                    <div className="col-span-2 grid grid-cols-3 gap-3 pt-2 border-t border-gray-100 dark:border-gray-700">
                                        <div>
                                            <label className="text-xs text-gray-500 block mb-1">{isRTL ? 'شفافية الطبقة' : 'Overlay Opacity'}</label>
                                            <input type="range" min="0" max="1" step="0.05" value={draft.heroOverlayOpacity ?? item.heroOverlayOpacity ?? 0.2} onChange={e => setDraft(p => ({ ...p, heroOverlayOpacity: parseFloat(e.target.value) }))} className="w-full accent-emerald-600" />
                                            <span className="text-xs text-gray-400 block text-center">{draft.heroOverlayOpacity ?? item.heroOverlayOpacity ?? 0.2}</span>
                                        </div>
                                        <div>
                                            <label className="text-xs text-gray-500 block mb-1">{isRTL ? 'ضبابية الجو' : 'Fog (px)'}</label>
                                            <input type="range" min="0" max="20" step="1" value={draft.heroBlurAmount ?? item.heroBlurAmount ?? 0} onChange={e => setDraft(p => ({ ...p, heroBlurAmount: parseInt(e.target.value) }))} className="w-full accent-emerald-600" />
                                            <span className="text-xs text-gray-400 block text-center">{draft.heroBlurAmount ?? item.heroBlurAmount ?? 0}px</span>
                                        </div>
                                        <div>
                                            <label className="text-xs text-gray-500 block mb-1">{isRTL ? 'لون الطبقة' : 'Overlay Color'}</label>
                                            <div className="flex items-center gap-2">
                                                <input type="color" value={draft.heroOverlayColor ?? item.heroOverlayColor ?? '#0369a1'} onChange={e => setDraft(p => ({ ...p, heroOverlayColor: e.target.value }))} className="w-8 h-8 rounded border cursor-pointer" />
                                                <input value={draft.heroOverlayColor ?? item.heroOverlayColor ?? '#0369a1'} onChange={e => setDraft(p => ({ ...p, heroOverlayColor: e.target.value }))} className="flex-1 px-2 py-1 rounded-lg border dark:bg-gray-800 dark:border-gray-700 text-xs" placeholder="#0369a1" />
                                            </div>
                                        </div>
                                    </div>
                                </div>
