...
export default async function ContainerTransportPage() {
    ...
    // Fetch hero image and overlay settings from SpecializedCargo CMS
    let heroImage = 'https://images.unsplash.com/photo-1605745341112-85968b19335b?auto=format&fit=crop&q=60&w=1920';
    let heroOverlayColor: string | null = null;
    let heroOverlayOpacity: number | null = null;
    let heroBlurAmount: number | null = null;
    try {
        const cargo = await prisma.specializedCargo.findUnique({
            where: { slug: 'container-transport' },
            select: { image: true, heroOverlayColor: true, heroOverlayOpacity: true, heroBlurAmount: true },
        });
        if (cargo?.image) heroImage = cargo.image;
        if (cargo?.heroOverlayColor) heroOverlayColor = cargo.heroOverlayColor;
        if (cargo?.heroOverlayOpacity != null) heroOverlayOpacity = cargo.heroOverlayOpacity;
        if (cargo?.heroBlurAmount != null) heroBlurAmount = cargo.heroBlurAmount;
    } catch {
